# Mentions légales — Coach Volley

**Dernière mise à jour : 20 septembre 2026**

---

## Éditeur de l'application

**[À COMPLÉTER : prénom et nom]**
Personne physique agissant à titre individuel, sans activité commerciale.

**Contact : [À COMPLÉTER : adresse e-mail]**

> *Remarque — Une personne physique qui édite un service en ligne à titre non
> professionnel peut se limiter à indiquer son nom et une adresse de contact,
> sans publier son adresse postale, à condition que le nom et les coordonnées
> de l'hébergeur soient mentionnés (article 6-III-2 de la LCEN). Ceux-ci
> figurent ci-dessous. Si le service devient payant ou est exploité à titre
> professionnel, cette section devra être complétée par l'adresse, le numéro
> SIREN et, le cas échéant, le numéro de TVA.*

**Directeur de la publication : [À COMPLÉTER : prénom et nom]**

---

## Hébergement des données

**Supabase, Inc.**
970 Toa Payoh North, #07-04, Singapour 318992
[supabase.com](https://supabase.com)

Les données de l'application sont hébergées dans un centre de données situé
dans l'**Union européenne**.

---

## Distribution de l'application

- **App Store** — Apple Inc., One Apple Park Way, Cupertino, CA 95014,
  États-Unis
- **Google Play** — Google Ireland Limited, Gordon House, Barrow Street,
  Dublin 4, Irlande

---

## Prestataire d'intelligence artificielle

**Anthropic PBC**
548 Market St, PMB 90375, San Francisco, CA 94104, États-Unis
[anthropic.com](https://www.anthropic.com)

Intervient uniquement pour générer les réponses du Coach IA. Le détail des
données transmises figure dans la
[politique de confidentialité](./politique-de-confidentialite.md), section 5.

---

## Propriété intellectuelle

L'ensemble des éléments de l'application — code source, interface, textes,
bibliothèque d'exercices, illustrations — est protégé par le droit d'auteur.
Toute reproduction ou représentation, totale ou partielle, sans autorisation
préalable est interdite.

---

## Données personnelles

Voir la [politique de confidentialité](./politique-de-confidentialite.md).

Autorité de contrôle compétente : **CNIL**, 3 place de Fontenoy, TSA 80715,
75334 Paris Cedex 07 — [www.cnil.fr](https://www.cnil.fr).

---

## Avertissement santé

Coach Volley n'est pas un dispositif médical. Voir l'article 4 des
[conditions générales d'utilisation](./conditions-generales-utilisation.md).
